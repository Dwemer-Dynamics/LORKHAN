...
local function report(result,command)
    ...
    if canonical then core.sendGlobalEvent('ALMSIVI_ACTION_RESULT',{result=canonical,action_name=command.name}) end
end
local function reportDelivery(command,status,reason)
    if command and state and core and core.sendGlobalEvent then
        core.sendGlobalEvent('ALMSIVI_SPEECH_STATUS',{actor=state.identity,media_id=command.media_id,
...
return {
    ...
    eventHandlers={
        ALMSIVI_ACTOR_ATTACH=function(command)
        ...
        ALMSIVI_ACTOR_REJECT=function(command)
