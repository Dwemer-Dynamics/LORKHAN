...
| Input, dialogue, and presentation | Typed player input | AUTOMATED | OpenMW TextEdit input uses bounded UTF-8 validation and routes through the canonical player-text pipeline. Optional Auto Chat uses a separate authenticated rewrite request, then existing player subtitle/TTS before submitting the rewritten text as the real turn. |
