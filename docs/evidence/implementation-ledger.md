...
| Foundation and flow | Streaming response experience | AUTOMATED | Strict native/Lua `response.complete` parsing feeds one bounded FIFO response lane. Lua runtime tests prove response normalization, dialogue-before-action dispatch, media attachment, generation fencing, terminal delivery, queued-action cancellation and playback-gated rechat; in-game timing remains unproven. |
...
| Input, dialogue, and presentation | Interrupt/skip/hard halt | AUTOMATED | Typed interruption, Stop Dialogue, recoverable Halt, hard halt and action-only halt now cancel the owned FIFO work, release media, fence generations and emit terminal queued receipts in no-game tests. In-game proof remains incomplete. |
