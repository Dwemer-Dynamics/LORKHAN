...
| Input, dialogue, and presentation | Automatic greeting, boredom, and combat barks | EXCLUDED | No runtime scheduler, request path, capability, or poller exists; presentation landmarks are disabled compatibility-only controls. |
...
| Intelligence and server product | Narrator and diary | AUTOMATED | Opt-in narrator profile/routing plus manual narrator, diary, and summary CRUD are available; timer-driven automatic diary generation remains excluded. |
