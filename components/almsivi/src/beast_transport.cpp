...
Result<WireRequest> serializeRequest(const BaseUrl& baseUrl, const OutboundRequest& request)
{
    ...
    switch (request.kind) {
        ...
        case RequestKind::init: {
            ...
            wire.body = "{\"schema\":\"almsivi.session.init.v1\",\"message_id\":" + escapeJson(init->ids.message.value())
        ...
        case RequestKind::turn: {
            ...
            wire.body = "{\"schema\":\"almsivi.turn.v1\",\"message_id\":" + escapeJson(turn->ids.message.value())
        ...
        case RequestKind::interruption: {
            ...
            wire.body = "{\"schema\":\"almsivi.interrupt.v1\",\"message_id\":" + escapeJson(interruption->message.value())
        ...
        case RequestKind::action_result: {
            ...
            wire.body = "{\"schema\":\"almsivi.action-result.v1\",\"message_id\":" + escapeJson(action->message.value())
        ...
        case RequestKind::dialogue_delivery_result: {
            ...
            wire.body = "{\"schema\":\"almsivi.dialogue-delivery-result.v1\",\"message_id\":" + escapeJson(delivery->message.value())
        ...
        case RequestKind::stt: {
            ...
            wire.fixedHeaders = {
                {"X-ALMSIVI-Schema", "almsivi.stt.request.v1"},
                {"X-ALMSIVI-Message-Id", stt->ids.message.value()},
                {"X-ALMSIVI-Request-Id", stt->ids.request.value()},
                {"X-ALMSIVI-Turn-Id", stt->ids.turn.value()},
                {"X-ALMSIVI-Session-Id", stt->ids.session.value()},
                {"X-ALMSIVI-Generation", std::to_string(stt->ids.generation.value())},
                {"X-ALMSIVI-Created-At", stt->createdAt},
                {"X-ALMSIVI-Codec", stt->codec},
                {"X-ALMSIVI-Language", stt->language},
                {"X-ALMSIVI-Audio-Bytes", std::to_string(stt->audio.size())},
                {"X-ALMSIVI-Sha256", stt->sha256},
            };
            break;
        }
...
Result<InboundResult> BeastTransport::execute(const OutboundRequest& request, std::stop_token cancellation)
{
    ...
    message.set(http::field::user_agent, "ALMSIVI/" + std::string(kClientVersion));
    ...
    message.set("X-ALMSIVI-Auth", "hmac-sha256-v1");
    message.set("X-ALMSIVI-Installation-Id", m_impl->installation.value());
    message.set("X-ALMSIVI-Timestamp", timestamp);
    message.set("X-ALMSIVI-Nonce", nonce);
    message.set("X-ALMSIVI-Content-SHA256", digest);
    message.set("X-ALMSIVI-Signature", hexBytes(hmacSha256(m_impl->token.macKey(), canonical)));
