...
void testProtocolEventResponses()
{
    ...
    if (parsed && parsed.value().events.size() == 7) {
        ...
        CHECK(intent && intent->intent.action == lorkhan::ActionId(kAction)
            ...
            && intent->intent.followupEnabled == true);
