    ...
    std::string_view body, const Headers& headers, json::ParseLimits limits)
{
    ...
    if (json::find(object.value(), "profile_id")) {
        auto parsedProfile = requireUuid(object.value(), "profile_id");
    ...
    std::string_view body, const Headers& headers, json::ParseLimits limits)
{
    ...
    if(json::find(object.value(),"comment_requested")){
        ...
        if(!value||(type.value()!="rpg_event"&&type.value()!="bored_event"&&type.value()!="quest_event"))return invalidSchemaValue<GameDataAcceptedResponse>("unexpected RPG commentary flag");
        comment=value.value();
    }
