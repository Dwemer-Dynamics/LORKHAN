...
- Treat Lua API revision 129 and the `lorkhan.*.v1` wire schemas as versioned compatibility
...
- Do not add generic HTTP, socket, shell, process, unrestricted filesystem, or dynamic-code APIs to
  OpenMW Lua. `openmw.lorkhan` exposes only the typed operations in `ENGINE-INTEGRATION-PLAN.md`.
...
- Keep vanilla dialogue and saves functional. LORKHAN is opt-in per profile and installed beside
  stock OpenMW.
