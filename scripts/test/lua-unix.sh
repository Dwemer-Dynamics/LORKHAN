...
TEST_ROOT=${ALMSIVI_LUA_TEST_ROOT:-$ROOT/tests/lua}
REQUIRED=${ALMSIVI_REQUIRE_LUA_TESTS:-0}
LUA=${LUA:-}
if [ -z "$LUA" ]; then
  for name in lua5.4 lua54 lua; do if command -v "$name" >/dev/null 2>&1; then LUA=$name; break; fi; done
fi
if [ ! -d "$TEST_ROOT" ]; then
...
found=0
find "$TEST_ROOT" -type f -name '*_test.lua' -print | LC_ALL=C sort | while IFS= read -r test; do found=1; "$LUA" "$test"; done
if ! find "$TEST_ROOT" -type f -name '*_test.lua' -print -quit | grep . >/dev/null; then
  ...
  printf 'skip: no *_test.lua files discovered; no Lua proof claimed\n'
fi
