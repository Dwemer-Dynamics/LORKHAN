...
-- Capture the bounded NPC metadata used to seed a server profile when auto-activation succeeds.
function M.actorProfile(actorIdentity, modules)
    modules=modules or loaded()
    local actor,reason=M.resolve(actorIdentity,modules)
    local npc=modules.types and modules.types.NPC
    local actorType=modules.types and modules.types.Actor
    if not actor or not npc or not safe(npc.objectIsInstance,actor) then
        return nil,reason or 'npc_profile_unavailable'
    end
    local record=safe(npc.record,actor)
    if not record then return nil,'npc_record_unavailable' end
    ...
    local disposition=math.max(0,math.min(100,math.floor(tonumber(safe(npc.getDisposition,actor,modules.self)) or 0)))
    local factionIds,seen={},{}
    for _,faction in ipairs(factions(actor,modules)) do
    ...
    local race=type(record.race)=='string' and record.race:sub(1,128) or 'Unknown'
    local class=type(record.class)=='string' and record.class:sub(1,128) or ''
    return {actor=actorIdentity,race=race~='' and race or 'Unknown',class=class,
        gender=record.isMale==true and 'male' or record.isMale==false and 'female' or 'unknown',
